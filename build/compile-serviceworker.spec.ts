...
describe('Compile ServiceWorker (esbuild plugin)', { concurrency: true }, async () => {
	async function readSwOrig() {
		const swOrig = await readFile(
			findEntrypoint(config, 'sw/main.ts')!,
			'utf8',
		);

		return (await esbuild.transform(swOrig, { loader: 'ts' })).code;
	}
	const mock_fsReadFile = mock.fn(async () => readSwOrig());
	...
	const { compileServiceWorkerPlugin: plugin } = await import('./compile-serviceworker.ts');
	...
	it('should compile the service worker file', (t) => {
		const inKey = 'docs/sw/main.js';
		const outKey = 'docs/sw.js';
		const { setup } = plugin();
		...
		t.test('on-disk (prod mode)', async (c) => {
			...
			assert.equal(renameFrom, swTransPath, 'rename from');
